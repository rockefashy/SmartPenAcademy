...
export const updateFeeStatusDeclaration: FunctionDeclaration = {
  ...
  parameters: {
    ...
    properties: {
      ...
      confirmationToken: {
        type: Type.STRING,
        description: 'Cryptographic single-use confirmation token generated by the server during draft confirmation. Required to execute fee adjustment.'
      }
    },
    required: ['newStatus']
  }
...
export const updateFeeStatusTool: AgentTool = {
  ...
  async execute(args: any, context: AgentToolContext): Promise<AgentToolResult> {
    ...
    const { feeId, studentNameOrId, period, newStatus, newAmount, notes, confirmed, confirmationToken } = validation.data;
    ...
    if (user?.role === ROLES.COACH) {
      if (newStatus === 'Waived' || newAmount !== undefined) {
        return {
          result: null,
          summary: `Access Denied: Coaches cannot waive fees or modify billing amounts. Only administrators can perform fee ledger adjustments.`,
          success: false
        };
      }
    ...
    if (!confirmed) {
      ...
      return {
        ...
        summary: `⚠️ **Confirmation Required: Modify Fee Record Status**\n\n` +
          ...
          `To proceed, please confirm this fee update. (Server Confirmation Token: \`${issuedToken}\`)`,
        success: true
      };
    }
