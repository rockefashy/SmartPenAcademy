...
export const deleteAttendanceRecordDeclaration: FunctionDeclaration = {
  ...
  parameters: {
    ...
    properties: {
      attendanceId: {
        type: Type.STRING,
        description: 'Exact ID of the attendance record to delete (optional if student and date are provided).'
      },
      studentNameOrId: {
        type: Type.STRING,
        description: 'Name or ID of the student whose attendance is being deleted.'
      },
      date: {
        type: Type.STRING,
        description: 'Class date in YYYY-MM-DD format (e.g. "2026-09-08").'
      },
      notes: {
        type: Type.STRING,
      ...
      confirmationToken: {
        type: Type.STRING,
        description: 'Cryptographic single-use confirmation token generated by the server during draft confirmation. Required to execute deletion.'
      }
    }
  }
};
...
export const deleteAttendanceRecordTool: AgentTool = {
  ...
  async execute(args: any, _context: AgentToolContext): Promise<AgentToolResult> {
    ...
    const { attendanceId, studentNameOrId, date, notes, confirmed, confirmationToken } = validation.data;
    ...
    const actionPayload = { attendanceId: record.id, studentId: record.student_id, date: record.date };

    // 3. Draft Confirmation (confirmed !== true)
    if (!confirmed) {
      const issuedToken = generateConfirmationToken(
        _context.user?.id || 'anonymous',
      ...
      return {
        ...
        summary: `⚠️ **Confirmation Required: Delete Attendance Record**\n\n` +
          ...
          `To proceed, please confirm this deletion. (Server Confirmation Token: \`${issuedToken}\`)`,
        success: true
      };
    }
