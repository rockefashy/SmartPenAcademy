...
description: Applies to backend error handling, typed AppError hierarchy, async route handlers, structured logging with Logger.get(), stdout/stderr streams, and exception propagation.
---
...
3. **Centralized Express Error Pipeline** in `server/middleware/errorHandler.ts`
   - `asyncHandler(fn)` route wrapper catches unhandled async rejections and forwards to `next(err)`.
...
Route handlers must not use copy-pasted inline `try { ... } catch (err) { res.status(500).json(...) }` blocks.

Handlers must:
- Be wrapped in `asyncHandler(fn)`, and
- Throw typed `AppError` instances directly.

Uncaught errors must flow naturally to the global `errorHandler` middleware.

---

### Rule #2: Automatic Recursive Secret Redaction

Sensitive keys (`password`, `token`, `authorization`, `cookie`, `apiKey`, `credential`, `session`, `bearer`, `hash`) and values matching JWTs or API keys must be recursively sanitized before writing to logs. Plaintext secrets must never appear in standard streams.

In addition, never log:
- Full JWTs (log only non-sensitive claims like `userId`, `role` if needed).
- Raw request bodies that may contain passwords or excessive PII.
- Stack traces to the client; stack traces go only to stderr via the logger.
...
Database query methods and service layers must explicitly distinguish between:
- **Legitimate empty query results** (query succeeds, zero rows match), and
- **Execution failures** (exceptions from DB or connection).

Requirements:

- **Legitimate empty results**:
  - Collections: return `[]`.
  - Single-entity lookups: return `null`.
  - This is correct behavior, per `GEMINI.md` “No Mock / No Fallback”.

- **Execution failures**:
  - Must NOT be silently swallowed into `[]` or `null`.
  - Must be logged via `Logger.get('DB')`.
  - Must be re-thrown as a typed `DatabaseError` so the API layer responds with an appropriate HTTP error and diagnostic context.
...
- **PostgreSQL Audit Trail (`recordAudit()` / `tool_audit_logs`)**: Authoritative, persistent, tamper-evident ledger in PostgreSQL (`Security.md` Section 15).
- **`Logger.get('AUDIT')`**: Real-time operational log stream to stdout for DevOps and monitoring visibility.

`recordAudit()` must emit to `Logger.get('AUDIT')` upon execution. The operational log stream does not replace PostgreSQL persistence.
...
For error handling and logging, this file is authoritative for Smart Pen Academy.