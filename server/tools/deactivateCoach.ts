...
export const deactivateCoachTool: AgentTool = {
  ...
  async execute(args: any, context: AgentToolContext): Promise<AgentToolResult> {
    ...
    const { coachNameOrId, dateOfLeaving, notes, confirmed, confirmationToken } = validation.data;
    ...
    if (!confirmed && coach.status === 'Inactive') {
    ...
    const actionPayload = { coachId: coach.id, effectiveDate };
