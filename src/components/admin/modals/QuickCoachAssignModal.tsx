      ...
      <div className="space-y-4">
        ...
        <div className="p-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs space-y-1">
          <p className="font-bold text-slate-800 flex items-center gap-2">
            <span>{student.firstName} ({student.gradeLevel || 'Standard Grade'})</span>
        ...
        {student.status === 'Inactive' && (
          <div className="p-3 bg-amber-50 border border-amber-300 text-amber-900 rounded-xl text-xs font-semibold flex items-center gap-2">
            ...
            <span>Cannot assign a coach to an inactive student. Please activate the student first.</span>
          </div>
        )}
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
            ...
            {coaches
              ...
              .map((c) => {
                ...
                const isInactiveCoach = c.status === 'Inactive';
                const isDisabled = student.status === 'Inactive' || (isInactiveCoach && !isCurrent);
                return (
                  <Button
                    key={c.id}
                    type="button"
