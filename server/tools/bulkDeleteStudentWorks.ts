...
export const bulkDeleteStudentWorksDeclaration: FunctionDeclaration = {
  ...
  parameters: {
    ...
    properties: {
      ...
      confirmationToken: {
        type: Type.STRING,
        description: 'Cryptographic single-use confirmation token generated by the server during draft confirmation. Required to execute bulk deletion.'
      }
    },
...
export const bulkDeleteStudentWorksTool: AgentTool = {
  ...
  async execute(args: any, context: AgentToolContext): Promise<AgentToolResult> {
    ...
    const resolvedIds = resolvedWorks.map(w => w.id).sort();
    const actionPayload = { resolvedIds };
    ...
    if (!tokenVerification.valid) {
      return {
        result: null,
        summary: `Security Error: Action rejected. ${tokenVerification.error || 'Invalid or missing confirmation token.'} Bulk deletion requires an authentic server-generated confirmation token.`,
        success: false
      };
    }
